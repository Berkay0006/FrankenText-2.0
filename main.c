...
void append_to_succs(char *token, char *succ) {
  ...
  size_t next_empty_index = succs_sizes[id];

  if (next_empty_index >= MAX_SUCCESSOR_COUNT) {
    printf("Successor array full.\n");
    exit(EXIT_FAILURE);
  }

  succs[id][next_empty_index] = succ;
  succs_sizes[id]++;
}
...
void tokenize_and_fill_succs(char *delimiters, char *str) {
  ...
  while (token != NULL) {
    token_id(token);
    if (previous != NULL)
    append_to_succs(previous, token);
...
char *generate_sentence(char *sentence, size_t sentence_size) {
  ...
  do {
    ...
    sentence_len_next = strlen(sentence) + strlen(next) + 2;
